...
function [solution, error] = newton(x0, f, tol, maxiter)
    ...
    %     iterations
    %   - The error is computed as the absolute difference between successive
    %     iterations

    % Forward difference step size (can be adjusted for better accuracy)
    h = 1e-7;
    ...
    % Check if derivative is too close to zero at initial guess
    if abs(diffy) < 1e-10
        error('Newton method fails: Derivative too close to zero at initial guess x0 = %.6f', x0);
    end
    
    x0v = zeros(1, maxiter);     % Array to store all iterations
    error = zeros(1, maxiter);   % Array to store errors
    x0v(1) = x0;                 % Store initial guess
    error(1) = inf;              % Initialize error (no previous iteration yet)
    iteration_number = 1;
    ...
    while error(iteration_number) > tol && iteration_number < maxiter
        ...
        % Compute error as absolute difference between successive iterations
        error(iteration_number+1) = abs(x0v(iteration_number+1) - x0v(iteration_number));
    
        % Print iteration information
        fprintf("%-i\t\t %-.6f\t %-.6f\t %-.6f\t %-.6e\n", ...
        ...
        % Check if derivative becomes too close to zero during iterations
        if abs(diffy) < 1e-10
            warning('Newton method may fail: Derivative too close to zero at iteration %d (x = %.6f)', ...
                iteration_number+1, x0v(iteration_number+1));
    ...
    if iteration_number >= maxiter && error(iteration_number) > tol
        warning('Maximum iterations (%d) reached without achieving desired tolerance (%.2e)', ...
            maxiter, tol);
    end

    % Trim output arrays to actual number of iterations used
    solution = x0v(1:iteration_number);
    error = error(1:iteration_number);
end