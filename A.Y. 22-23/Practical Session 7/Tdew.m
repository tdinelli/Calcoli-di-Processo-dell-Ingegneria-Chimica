...
function [T, fval, exitFlag] = Tdew(P, speciesname, comp, FG, solver)
...
options = optimset('Display','iter','PlotFcns',...
    ...
    function sum = fdew(T)
        ...
        % NOTE THAT ALL THE VARIABLES AVAILABLE IN THE FUNCTION Tdew 
        ...
        for i = 1:nname  % this for cycle calculates and add to sum each
            ...
            sum = sum + P * comp(i) / PVap4Comp(T,speciesname(i,:));
    ...
    if solver=='solve'
        [T, fval, exitFlag] = fsolve(@fdew, FG, options);
    elseif solver=='fzero'
        [T, fval, exitFlag] = fzero(@fdew, FG, options);
    end
    %else
